...
First, clone this repo with the name of the desired provider in place of `keycloak`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-keycloak
```

Second, replace references to `keycloak` with the name of your provider:
...
- `GO111MODULE=on go get github.com/terraform-providers/terraform-provider-keycloak` (where `keycloak` is the name of the provider)
...
The following configuration points are available for the `keycloak` provider:

- `keycloak:apiKey` (environment: `XYZ_API_KEY`) - the API key for `keycloak`
- `keycloak:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
